...
; Leaves strlen in X
!macro to_petscii .inout {
  ldx #0
- lda .inout,x
  beq +
  tay
  lda ascii_to_petscii_table,y
  sta .inout,x
  inx
  jmp -
+
...
; Copies a string from .in to .out converting PETSCII to ASCII
; NOTE: .out is UNTERMINATED!
!macro to_ascii2 .in, .out {
  ldx #0
- lda .in,x
  beq +
  tay
  lda petscii_to_ascii_table,y
  sta .out,x
  inx
  jmp -
+
...
; Copies a string from .in to .out (starting at .out_start) converting PETSCII to ASCII
; NOTE: .out is UNTERMINATED!
!macro to_ascii3 .in, .out, .out_start {
  ldx #0
- lda .in,x
  beq +
  tay
  ...
  ; Compute output offset in Y (.out_start + X)
  pha
  txa
  clc
  adc .out_start
